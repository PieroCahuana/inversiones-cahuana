...
export function RegisterPage() {
  ...
  const inputClass = "mt-2 w-full rounded-xl border border-[#dce4ef] bg-[#f8fafc] px-4 py-3.5 outline-none transition focus:border-[#1454d8] focus:ring-4 focus:ring-[#1454d8]/10";
    ...
    <main className="site-container py-10 sm:py-16">
      <div className="mx-auto grid max-w-6xl overflow-hidden rounded-[24px] border border-[#e1e7ef] bg-white shadow-[0_30px_90px_rgba(13,38,76,0.12)] lg:grid-cols-[0.78fr_1.22fr]">
        ...
        <section className="p-7 sm:p-10 lg:p-12">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-[#1454d8]">Registro</p>
          ...
          <form onSubmit={handleSubmit} className="mt-7 grid gap-5 sm:grid-cols-2">
            ...
            <button disabled={isSubmitting} className="rounded-xl bg-[#1454d8] px-5 py-4 text-sm font-black text-white hover:bg-[#0d45bd] disabled:opacity-60 sm:col-span-2">{isSubmitting ? "Creando cuenta..." : "Crear mi cuenta"}</button>
          </form>
          <p className="mt-6 text-center text-sm text-[#6c798a]">¿Ya tienes una cuenta? <Link to="/login" state={location.state} className="font-black text-[#1454d8] hover:underline">Inicia sesión</Link></p>
        </section>
      </div>
    </main>
  );
