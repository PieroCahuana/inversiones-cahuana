...
export function AdminOrdersPage() {
  ...
  return <>
    ...
    {!orders.data?.length ? <AdminEmpty>No hay pedidos para los filtros seleccionados.</AdminEmpty> : <div className="overflow-hidden rounded-2xl border border-[#dfe7ef] bg-white"><div className="divide-y divide-[#edf1f5]">{orders.data.map((order) => <Link key={order.id} to={`/admin/orders/${order.order_number}`} className="grid items-center gap-3 px-5 py-4 transition hover:bg-[#f8faff] md:grid-cols-[minmax(220px,1fr)_150px_150px_120px_30px]"><div className="flex min-w-0 items-center gap-3"><div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#eaf7fc] text-[#249fd3]"><PackageSearch size={19} /></div><div><p className="text-sm font-black">{order.order_number}</p><p className="mt-1 text-xs text-[#7b8999]">{new Date(order.created_at).toLocaleString("es-PE")} · {order.total_items} artículo(s)</p></div></div><span className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-black ${orderStatusClasses[order.status]}`}>{order.status_display}</span><span className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-black ${paymentStatusClasses[order.payment_status]}`}>{order.payment_status_display}</span><strong className="text-sm">{money.format(Number(order.total))}</strong><ArrowRight size={17} className="text-[#8a97a7]" /></Link>)}</div></div>}
  </>;
}
