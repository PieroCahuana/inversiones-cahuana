...
const categories = [
  { title: "Laptops", subtitle: "Trabajo y estudio", href: "/products?category=laptops", icon: Laptop, accent: "bg-[#e8f0ff] text-[#1454d8]" },
    ...
    <main className="bg-white">
        ...
        <div className="grid overflow-hidden rounded-2xl border border-[#e1e7ef] bg-white shadow-[0_15px_45px_rgba(13,38,76,0.1)] sm:grid-cols-2 lg:grid-cols-4">
          {benefits.map(({ icon: Icon, title, description }, index) => (
            <article key={title} className={`flex items-center gap-4 px-6 py-5 ${index ? "border-t border-[#e8edf3] sm:border-l sm:border-t-0" : ""}`}>
              <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#edf3ff] text-[#1454d8]"><Icon size={21} /></div>
      ...
      <section className="site-container py-16 lg:py-20">
        <div className="flex items-end justify-between gap-6">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-[#1454d8]">Compra por categoría</p>
            <h2 className="mt-3 text-3xl font-black tracking-[-0.04em] text-[#071d41] sm:text-4xl">Encuentra lo que necesitas</h2>
          </div>
          <Link to="/products" className="hidden items-center gap-2 text-sm font-black text-[#1454d8] hover:underline sm:flex">Ver todo <ArrowRight size={17} /></Link>
        </div>
        ...
        <div className="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map(({ title, subtitle, href, icon: Icon, accent }) => (
            <Link key={title} to={href} className="group rounded-[22px] border border-[#e2e8f0] bg-white p-5 text-center transition duration-300 hover:-translate-y-1 hover:border-[#b9cae4] hover:shadow-[0_18px_45px_rgba(17,55,107,0.11)]">
              ...
              <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-black text-[#1454d8]">Ver productos <ChevronRight size={16} /></span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-[#e3e9f0] bg-[#f7f9fc] py-6">
        <div className="site-container flex flex-wrap items-center justify-center gap-x-12 gap-y-4">
          ...
          {brands.map((brand) => <span key={brand} className="text-lg font-black tracking-[0.1em] text-[#6e7b8d] transition hover:text-[#1454d8]">{brand}</span>)}
        </div>
      </section>
      ...
      <section className="bg-white py-16 lg:py-20">
        <div className="site-container">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-[#1454d8]">Recomendados para ti</p>
            ...
            <Link to="/products" className="inline-flex w-fit items-center gap-2 rounded-xl bg-[#1454d8] px-5 py-3 text-sm font-black text-white hover:bg-[#0d45bd]">Catálogo completo <ArrowRight size={17} /></Link>
          </div>
      ...
      <section id="soporte" className="site-container pb-20">
        <div className="grid gap-5 lg:grid-cols-3">
          <article className="relative overflow-hidden rounded-[24px] bg-[#1454d8] p-8 text-white lg:col-span-2 sm:p-10">
            ...
            <div className="relative max-w-2xl">
              ...
              <a href="https://wa.me/51954107191?text=Hola%2C%20necesito%20información%20sobre%20soporte%20técnico." target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3.5 text-sm font-black text-[#1454d8] hover:bg-[#ffd500] hover:text-[#071d41]">Solicitar soporte <ArrowRight size={17} /></a>
            </div>
          </article>
          <article className="rounded-[24px] border border-[#e1e7ef] bg-[#f7f9fc] p-8 sm:p-10">
            <MapPin size={29} className="text-[#1454d8]" />
            <p className="mt-7 text-xs font-black uppercase tracking-[0.2em] text-[#1454d8]">Visítanos</p>
