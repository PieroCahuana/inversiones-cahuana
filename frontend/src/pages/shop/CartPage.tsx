...
export function CartPage() {
    ...
    return (
      <main className="site-container py-16 sm:py-20">
        <div className="mx-auto max-w-2xl rounded-[24px] border border-[#e1e7ef] bg-white px-7 py-14 text-center shadow-[0_24px_70px_rgba(13,38,76,0.09)]">
          <div className="mx-auto flex size-20 items-center justify-center rounded-full bg-[#eaf7fc] text-[#249fd3]"><LockKeyhole size={32} /></div>
          ...
          <Link to="/login" state={{ from: "/cart" }} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-[#249fd3] px-6 py-3.5 text-sm font-black text-white hover:bg-[#167fac]">Iniciar sesión <ArrowRight size={17} /></Link>
        </div>
      </main>
    );
  }
  ...
  if (cartQuery.isPending) {
    return <main className="site-container flex min-h-[520px] items-center justify-center"><div className="text-center"><LoaderCircle size={36} className="mx-auto animate-spin text-[#249fd3]" /><p className="mt-4 text-sm font-bold text-[#6c798a]">Cargando tu carrito...</p></div></main>;
  }
    ...
    return (
      <main className="site-container py-16 text-center">
        <div className="mx-auto max-w-xl rounded-[24px] border border-[#f0d5d0] bg-[#fff7f5] p-10">
          ...
          {unauthorized ? <Link to="/login" state={{ from: "/cart" }} className="mt-6 inline-flex rounded-xl bg-[#249fd3] px-5 py-3 text-sm font-black text-white">Iniciar sesión</Link> : <button type="button" onClick={() => cartQuery.refetch()} className="mt-6 rounded-xl bg-[#249fd3] px-5 py-3 text-sm font-black text-white">Reintentar</button>}
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="site-container py-14 sm:py-20">
        <div className="mx-auto max-w-3xl rounded-[24px] border border-[#e1e7ef] bg-white px-6 py-14 text-center shadow-[0_24px_70px_rgba(13,38,76,0.09)]">
          <div className="mx-auto flex size-20 items-center justify-center rounded-full bg-[#eaf7fc] text-[#249fd3]"><ShoppingBag size={32} /></div>
          ...
          <Link to="/products" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-[#249fd3] px-6 py-3.5 text-sm font-black text-white hover:bg-[#167fac]">Explorar productos <ArrowRight size={17} /></Link>
        </div>
      </main>
    );
  }
    ...
    <main className="bg-[#f7f9fc] py-10 sm:py-14">
      <div className="site-container">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#249fd3]">Tu selección</p><h1 className="mt-2 text-4xl font-black tracking-[-0.045em] text-[#071d41] sm:text-5xl">Mi carrito</h1><p className="mt-2 text-sm text-[#6c798a]">{cart.total_items} {cart.total_items === 1 ? "producto" : "productos"} en tu carrito</p></div>
          <Link to="/products" className="flex items-center gap-2 text-sm font-black text-[#249fd3] hover:underline"><ArrowLeft size={16} /> Seguir comprando</Link>
        </div>
        ...
        <div className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                ...
                return (
                  <article key={item.id} className={`grid gap-4 p-5 transition sm:grid-cols-[120px_minmax(0,1fr)_auto] sm:p-6 ${isRemoving ? "opacity-45" : ""}`}>
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#249fd3]">{item.product.brand}</p>
                      <Link to={`/products/${item.product.slug}`} className="mt-2 block text-lg font-black leading-6 text-[#102a4e] hover:text-[#249fd3]">{item.product.name}</Link>
          ...
          <aside className="sticky top-[190px] overflow-hidden rounded-[20px] border border-[#dce4ef] bg-white shadow-[0_18px_45px_rgba(13,38,76,0.08)]">
            ...
            <div className="p-6">
              ...
              <Link to="/checkout" className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#249fd3] px-5 py-3.5 text-sm font-black text-white transition hover:bg-[#167fac]"><ShieldCheck size={18} /> Continuar al checkout</Link>
              ...
              <div className="mt-5 space-y-3 border-t border-[#e8edf3] pt-5 text-xs font-bold text-[#637286]"><p className="flex items-center gap-2"><ShieldCheck size={16} className="text-[#249fd3]" /> Compra asociada a tu cuenta</p><p className="flex items-center gap-2"><Headphones size={16} className="text-[#249fd3]" /> Asesoría antes de confirmar</p></div>
            </div>
          </aside>
        </div>
      </div>
