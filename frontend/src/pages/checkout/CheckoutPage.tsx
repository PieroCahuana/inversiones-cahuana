...
export function CheckoutPage() {
  ...
  if (cartQuery.isPending) return <main className="flex min-h-[60vh] items-center justify-center"><LoaderCircle size={36} className="animate-spin text-[#1454d8]" /></main>;

  if (cartQuery.isError) return <main className="site-container py-20 text-center"><h1 className="text-2xl font-black text-[#102a4e]">No pudimos preparar el checkout</h1><button type="button" onClick={() => cartQuery.refetch()} className="mt-5 rounded-xl bg-[#1454d8] px-5 py-3 text-sm font-black text-white">Reintentar</button></main>;

  const cart = cartQuery.data;
  if (!cart.items.length) return <main className="site-container py-20 text-center"><PackageCheck size={40} className="mx-auto text-[#1454d8]" /><h1 className="mt-5 text-3xl font-black text-[#102a4e]">Tu carrito está vacío</h1><p className="mt-2 text-[#6c798a]">Agrega productos antes de iniciar el checkout.</p><Link to="/products" className="mt-6 inline-flex rounded-xl bg-[#1454d8] px-5 py-3 text-sm font-black text-white">Ir al catálogo</Link></main>;
  ...
  const inputClass = "mt-2 w-full rounded-xl border border-[#d8e1ec] bg-[#f8fafc] px-4 py-3.5 text-sm outline-none transition focus:border-[#1454d8] focus:ring-4 focus:ring-[#1454d8]/10";

  return (
    <main className="bg-[#f7f9fc] py-9 sm:py-12">
      <div className="site-container">
        <Link to="/cart" className="inline-flex items-center gap-2 text-sm font-black text-[#1454d8] hover:underline"><ArrowLeft size={16} /> Volver al carrito</Link>
        <div className="mt-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#1454d8]">Compra segura</p><h1 className="mt-2 text-4xl font-black tracking-[-0.045em] text-[#071d41] sm:text-5xl">Finaliza tu pedido</h1></div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
          <div className="space-y-6">
            <section className="rounded-[20px] border border-[#e1e7ef] bg-white p-6 sm:p-8">
              <div className="flex items-center gap-3 border-b border-[#e8edf3] pb-5"><div className="flex size-11 items-center justify-center rounded-xl bg-[#edf3ff] text-[#1454d8]"><MapPin size={21} /></div><div><p className="text-xs font-black uppercase tracking-[0.15em] text-[#1454d8]">Paso 1</p><h2 className="mt-1 text-xl font-black text-[#183353]">Datos de entrega</h2></div></div>
            ...
            <section className="rounded-[20px] border border-[#e1e7ef] bg-white p-6 sm:p-8">
              <div className="flex items-center gap-3 border-b border-[#e8edf3] pb-5"><div className="flex size-11 items-center justify-center rounded-xl bg-[#edf3ff] text-[#1454d8]"><Landmark size={21} /></div><div><p className="text-xs font-black uppercase tracking-[0.15em] text-[#1454d8]">Paso 2</p><h2 className="mt-1 text-xl font-black text-[#183353]">Método de pago</h2></div></div>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {paymentMethods.map(({ value, label, description, icon: Icon }, index) => <label key={value} className="group cursor-pointer"><input type="radio" name="payment_method" value={value} required defaultChecked={index === 0} className="peer sr-only" /><span className="flex items-center gap-3 rounded-2xl border-2 border-[#e1e7ef] p-4 transition peer-checked:border-[#1454d8] peer-checked:bg-[#f3f7ff]"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#edf3ff] text-[#1454d8]"><Icon size={19} /></span><span><strong className="block text-sm text-[#183353]">{label}</strong><small className="mt-1 block text-[#7b8999]">{description}</small></span></span></label>)}
              </div>
          ...
          <aside className="sticky top-[190px] overflow-hidden rounded-[20px] border border-[#dce4ef] bg-white shadow-[0_18px_45px_rgba(13,38,76,0.08)]">
            ...
            <div className="border-t border-[#e1e7ef] p-6">
              <div className="mb-5"><label className="text-xs font-black text-[#334862]">¿Tienes un cupón?</label><div className="mt-2 flex gap-2"><input value={couponCode} onChange={(event) => { setCouponCode(event.target.value.toUpperCase()); couponMutation.reset(); }} placeholder="CÓDIGO" className="h-10 min-w-0 flex-1 rounded-xl border border-[#d8e1ec] px-3 text-xs font-black uppercase outline-none focus:border-[#1454d8]" /><button type="button" onClick={() => couponMutation.mutate(couponCode)} disabled={!couponCode.trim() || couponMutation.isPending} className="rounded-xl bg-[#edf3ff] px-3 text-xs font-black text-[#1454d8] disabled:opacity-50">Aplicar</button></div>{couponMutation.isError && <p className="mt-2 text-[10px] font-bold text-red-600">Cupón inválido, vencido o no aplicable.</p>}{couponMutation.data && <p className="mt-2 text-[10px] font-bold text-emerald-700">Cupón {couponMutation.data.code}: ahorras {formatPrice(couponMutation.data.discount_amount)}</p>}</div>
              ...
              <button disabled={checkoutMutation.isPending} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1454d8] px-5 py-4 text-sm font-black text-white shadow-[0_10px_24px_rgba(20,84,216,0.22)] hover:bg-[#0d45bd] disabled:opacity-60">{checkoutMutation.isPending ? <><LoaderCircle size={18} className="animate-spin" /> Creando pedido...</> : <><BadgeCheck size={18} /> Confirmar pedido</>}</button>
              <div className="mt-4 flex items-center justify-center gap-2 text-[11px] font-semibold text-[#7b8999]"><LockKeyhole size={14} className="text-[#1454d8]" /> Tus datos se envían de forma segura</div>
              <div className="mt-4 flex items-center justify-center gap-2 text-center text-[11px] font-semibold text-[#7b8999]"><Truck size={14} className="shrink-0 text-[#1454d8]" /> {storeSettings?.checkout_message || "Coordinaremos la entrega contigo"}</div>
            </div>
          </aside>
        </form>
      </div>
