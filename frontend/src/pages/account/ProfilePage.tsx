...
export function ProfilePage() {
  ...
  const inputClass = "mt-2 w-full rounded-xl border border-[#d8e1ec] bg-[#f8fafc] px-4 py-3.5 outline-none transition focus:border-[#249fd3] focus:ring-4 focus:ring-[#249fd3]/10";

  return (
    <main className="bg-[#f7f9fc] py-10 sm:py-14">
      <div className="site-container max-w-5xl">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-[#249fd3]">Mi cuenta</p>
        ...
        <div className="mt-8 grid items-start gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="overflow-hidden rounded-[20px] border border-[#dfe7f0] bg-white">
            <div className="bg-[#071d41] p-7 text-center text-white"><div className="mx-auto flex size-20 items-center justify-center rounded-full bg-[#249fd3] text-2xl font-black">{initials}</div><h2 className="mt-4 text-xl font-black">{user.full_name || user.username}</h2><p className="mt-1 break-all text-sm text-[#b9c8dc]">{user.email}</p></div>
            <div className="space-y-4 p-6 text-sm text-[#637286]"><p className="flex items-center gap-3"><Mail size={17} className="text-[#249fd3]" /> {user.email}</p><p className="flex items-center gap-3"><Phone size={17} className="text-[#249fd3]" /> {user.phone || "Sin teléfono"}</p><p className="flex items-center gap-3 capitalize"><CalendarDays size={17} className="text-[#249fd3]" /> Miembro desde {memberSince}</p><p className="flex items-center gap-3"><BadgeCheck size={17} className={user.is_verified ? "text-emerald-600" : "text-[#8a97a7]"} /> {user.is_verified ? "Cuenta verificada" : "Verificación pendiente"}</p></div>
          </aside>
          ...
          <section className="rounded-[20px] border border-[#dfe7f0] bg-white p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="mt-6 grid gap-5 sm:grid-cols-2">
              ...
              <button disabled={isSaving} className="flex items-center justify-center gap-2 rounded-xl bg-[#249fd3] px-5 py-3.5 text-sm font-black text-white hover:bg-[#167fac] disabled:opacity-60 sm:col-span-2 sm:justify-self-end"><Save size={17} /> {isSaving ? "Guardando..." : "Guardar cambios"}</button>
            </form>
          </section>
        </div>
      </div>
