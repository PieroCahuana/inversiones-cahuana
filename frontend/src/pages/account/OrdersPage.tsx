  ...
  return (
    <main className="bg-[#f7f9fc] py-10 sm:py-14">
      <div className="site-container max-w-6xl">
        ...
        <div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="text-4xl font-black tracking-[-0.045em] text-[#071d41] sm:text-5xl">Mis pedidos</h1><p className="mt-3 text-[#6c798a]">Consulta el estado y los detalles de tus compras.</p></div><Link to="/products" className="inline-flex w-fit items-center gap-2 rounded-xl bg-[#249fd3] px-5 py-3 text-sm font-black text-white"><ShoppingBag size={17} /> Seguir comprando</Link></div>

        <div className="mt-8 flex flex-col gap-3 rounded-[20px] border border-[#e1e7ef] bg-white p-4 sm:flex-row sm:items-center">
          <form onSubmit={submitSearch} className="flex h-11 min-w-0 flex-1 items-center rounded-xl border border-[#d8e1ec] bg-[#f8fafc] px-3 focus-within:border-[#249fd3]"><Search size={17} className="text-[#7b8999]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por número de pedido" className="min-w-0 flex-1 bg-transparent px-3 text-sm outline-none" /><button className="text-xs font-black text-[#249fd3]">Buscar</button></form>
        ...
        {ordersQuery.isError && <div className="mt-6 rounded-[20px] border border-red-200 bg-red-50 p-10 text-center"><p className="font-black text-red-700">No pudimos cargar tus pedidos.</p><button type="button" onClick={() => ordersQuery.refetch()} className="mt-4 text-sm font-black text-[#249fd3]">Reintentar</button></div>}
        {ordersQuery.data?.length === 0 && <div className="mt-6 rounded-[20px] border border-dashed border-[#cbd6e2] bg-white p-12 text-center"><PackageOpen size={38} className="mx-auto text-[#94a2b3]" /><h2 className="mt-4 text-xl font-black text-[#334862]">No encontramos pedidos</h2><p className="mt-2 text-sm text-[#718094]">Cuando realices una compra aparecerá aquí.</p>{searchParams.size > 0 ? <button type="button" onClick={() => { setSearch(""); setSearchParams({}); }} className="mt-5 text-sm font-black text-[#249fd3] hover:underline">Limpiar filtros</button> : <Link to="/products" className="mt-5 inline-flex rounded-xl bg-[#249fd3] px-5 py-3 text-sm font-black text-white">Explorar catálogo</Link>}</div>}
