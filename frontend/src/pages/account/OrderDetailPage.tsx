...
export function OrderDetailPage() {
  ...
  if (orderQuery.isError || !orderQuery.data) {
    ...
    return <main className="site-container py-20 text-center"><ReceiptText size={40} className="mx-auto text-[#1454d8]" /><h1 className="mt-5 text-3xl font-black text-[#102a4e]">{notFound ? "Este pedido no existe" : "No pudimos cargar el pedido"}</h1><Link to="/orders" className="mt-6 inline-flex rounded-xl bg-[#1454d8] px-5 py-3 text-sm font-black text-white">Volver a mis pedidos</Link></main>;
  }
  ...
  return (
    <main className="bg-[#f7f9fc] py-9 sm:py-12">
      <div className="site-container max-w-6xl">
        <Link to="/orders" className="inline-flex items-center gap-2 text-sm font-black text-[#1454d8] hover:underline"><ArrowLeft size={16} /> Mis pedidos</Link>
        ...
        <section className="mt-5 overflow-hidden rounded-[24px] border border-[#dce4ef] bg-white shadow-[0_18px_50px_rgba(13,38,76,0.07)]">
          ...
          <div className="p-6 sm:p-8">
            {order.status === "cancelled" ? <div className="rounded-2xl bg-red-50 p-5 text-center font-bold text-red-700">Este pedido fue cancelado.</div> : <div className="grid grid-cols-5"><div className="col-span-5 mb-3 flex justify-between">{activeOrderSteps.map((step, index) => { const active = index <= currentStep; return <div key={step} className="flex w-1/5 flex-col items-center text-center"><div className={`relative z-10 flex size-9 items-center justify-center rounded-full ${active ? "bg-[#1454d8] text-white" : "bg-[#e3e9f0] text-[#8a97a7]"}`}>{active ? <BadgeCheck size={18} /> : <Clock3 size={16} />}</div><span className={`mt-2 text-[10px] font-black sm:text-xs ${active ? "text-[#1454d8]" : "text-[#8a97a7]"}`}>{stepLabels[step]}</span></div>})}</div><div className="col-span-5 mx-[10%] -mt-[50px] mb-10 h-1 bg-[#e3e9f0]"><div className="h-full bg-[#1454d8] transition-all" style={{ width: `${Math.max(0, currentStep) / (activeOrderSteps.length - 1) * 100}%` }} /></div></div>}
          </div>
        </section>

        <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="overflow-hidden rounded-[20px] border border-[#e1e7ef] bg-white"><div className="border-b border-[#e8edf3] px-6 py-5"><h2 className="font-black text-[#183353]">Productos del pedido</h2></div><div className="divide-y divide-[#e8edf3]">{order.items.map((item) => <div key={item.id} className="flex items-center gap-4 p-5 sm:p-6"><div className="flex size-14 shrink-0 items-center justify-center rounded-xl bg-[#edf3ff] text-[#1454d8]"><PackageCheck size={22} /></div><div className="min-w-0 flex-1"><Link to={`/products/${item.product_slug}`} className="font-black text-[#102a4e] hover:text-[#1454d8]">{item.product_name}</Link><p className="mt-1 text-xs text-[#7b8999]">SKU: {item.product_sku} · {item.quantity} × {formatPrice(item.unit_price)}</p></div><p className="font-black text-[#071d41]">{formatPrice(item.subtotal)}</p></div>)}</div></section>

          <div className="space-y-6">
            <PaymentReceiptPanel order={order} />
            <aside className="rounded-[20px] border border-[#e1e7ef] bg-white p-6"><h2 className="flex items-center gap-2 font-black text-[#183353]"><ReceiptText size={19} className="text-[#1454d8]" /> Resumen</h2><div className="mt-5 space-y-3 text-sm text-[#637286]"><div className="flex justify-between"><span>Subtotal</span><span>{formatPrice(order.subtotal)}</span></div><div className="flex justify-between"><span>Envío</span><span>{Number(order.shipping_cost) === 0 ? "Gratis" : formatPrice(order.shipping_cost)}</span></div><div className="flex justify-between border-t border-[#e1e7ef] pt-4 text-base font-black text-[#071d41]"><span>Total</span><span>{formatPrice(order.total)}</span></div></div><div className="mt-5 rounded-xl bg-[#f3f7fd] p-4 text-xs text-[#52647a]"><strong className="block text-[#183353]">Método de pago</strong><span className="mt-1 block">{order.payment_method_display}</span></div></aside>
            <aside className="rounded-[20px] border border-[#e1e7ef] bg-white p-6"><h2 className="flex items-center gap-2 font-black text-[#183353]"><MapPin size={19} className="text-[#1454d8]" /> Entrega</h2><p className="mt-4 text-sm font-black text-[#334862]">{order.recipient_name}</p><p className="mt-2 text-sm leading-6 text-[#637286]">{order.address}<br />{order.district}, {order.province}<br />{order.department}<br />Tel. {order.recipient_phone}</p>{order.address_reference && <p className="mt-3 text-xs text-[#7b8999]">Referencia: {order.address_reference}</p>}</aside>
