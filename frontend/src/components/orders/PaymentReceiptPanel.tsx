...
export function PaymentReceiptPanel({ order }: { order: Order }) {
  ...
  return <aside className="rounded-[20px] border border-[#e1e7ef] bg-white p-6"><h2 className="flex items-center gap-2 font-black text-[#183353]"><FileCheck2 size={19} className="text-[#249fd3]" /> Comprobante de pago</h2>{receipt && <div className="mt-4 rounded-xl bg-[#f5f8fc] p-4"><div className="flex items-center justify-between gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${statusClass[receipt.status]}`}>{receipt.status_display}</span><a href={receipt.file_url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs font-black text-[#249fd3]"><Paperclip size={14} /> Ver archivo</a></div>{receipt.review_note && <p className="mt-3 text-xs leading-5 text-[#65758a]"><strong>Respuesta:</strong> {receipt.review_note}</p>}</div>}{showForm && <form onSubmit={submit} className="mt-4 grid gap-3"><p className="text-xs leading-5 text-[#6f7f92]">{receipt ? "Corrige el comprobante y vuelve a enviarlo." : "Adjunta una captura o PDF para confirmar tu pago."}</p><label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-[#b9c8da] bg-[#f8faff] p-3 text-xs font-bold"><FileUp size={17} /><input name="file" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required className="min-w-0" /></label><input name="customer_note" maxLength={255} placeholder="N.º de operación o nota (opcional)" className="h-10 rounded-xl border border-[#dce4ef] px-3 text-xs outline-none focus:border-[#249fd3]" />{error && <p className="rounded-lg bg-red-50 p-2 text-xs text-red-700">{error}</p>}<button disabled={upload.isPending} className="rounded-xl bg-[#249fd3] px-4 py-3 text-xs font-black text-white disabled:opacity-60">{upload.isPending ? "Enviando..." : receipt ? "Enviar nuevo comprobante" : "Enviar comprobante"}</button></form>}</aside>;
}
