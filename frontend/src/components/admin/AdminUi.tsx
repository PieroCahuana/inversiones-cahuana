...
export function AdminPageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: ReactNode }) {
  return <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#1454d8]">{eyebrow}</p><h1 className="mt-2 text-3xl font-black tracking-[-0.035em] text-[#102a4e] sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c798a]">{description}</p></div>{action}</div>;
}

export function AdminSearch({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder: string }) {
  return <label className="flex h-11 min-w-0 items-center gap-2 rounded-xl border border-[#dce4ed] bg-white px-3 focus-within:border-[#1454d8]"><Search size={17} className="shrink-0 text-[#8190a3]" /><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="w-full bg-transparent text-sm font-medium outline-none" /></label>;
}
...
export function AdminLoading() { return <div className="flex min-h-72 items-center justify-center"><LoaderCircle className="animate-spin text-[#1454d8]" size={34} /></div>; }
...
export const fieldClass = "mt-1.5 h-11 w-full rounded-xl border border-[#dbe3ec] bg-white px-3 text-sm font-medium outline-none transition focus:border-[#1454d8] focus:ring-3 focus:ring-[#1454d8]/10";
export const textareaClass = "mt-1.5 min-h-24 w-full resize-y rounded-xl border border-[#dbe3ec] bg-white px-3 py-2.5 text-sm font-medium outline-none transition focus:border-[#1454d8] focus:ring-3 focus:ring-[#1454d8]/10";
export const primaryButton = "inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#1454d8] px-5 text-sm font-black text-white shadow-[0_8px_20px_rgba(20,84,216,0.2)] transition hover:bg-[#0d45bd] disabled:opacity-50";
